...
export async function handleScrapeVideo(
  ...
  if (!data) {
    sendResponse({ status: "error", message: "Failed to fetch video data (Check API Key)" });
    return;
  }
  ...
  const transcriptText = data.transcript_only_text || data.transcript?.map((s: any) => s.text).join(" ") || null;
