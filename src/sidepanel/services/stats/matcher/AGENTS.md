...
- Keep the sidepanel matcher copy aligned with `llm-harness-js/src/stats`.
- Do not simplify matcher internals to suit current consumers; consume less data later rather than changing the copied matcher logic.
- Restrict sidepanel-specific edits here to compatibility seams only when the browser/runtime requires them.
