...
const BUNDLED_PROVIDER_LOGO_ASSETS = new Set(Object.values(PROVIDER_LOGO_ASSET_BY_PROVIDER));
...
function toBundledProviderLogoUrl(asset: string | null | undefined): string | null {
...
function bundledLogoAssetFromUrl(logoUrl: string | null | undefined): string | null {
...
function toAbsoluteArtificialAnalysisLogoUrl(logoUrl: string | null | undefined): string | null {
  const logoValue = nonEmptyString(logoUrl);
  if (!logoValue) {
    return null;
  }
