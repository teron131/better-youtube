...
export type LlmStatsModelMetadata = Pick<AvailableModel, "intelligenceScore" | "speedMetric">;
...
const MODEL_ATLAS_STATS_URL = "https://llm-stats.vercel.app/api/llm-stats?view=core";

let llmStatsModelMetadataPromise: Promise<LlmStatsModelMetadataIndex> | null = null;
...
export async function fetchLlmStatsModelMetadataIndex(): Promise<LlmStatsModelMetadataIndex> {
  try {
    if (!llmStatsModelMetadataPromise) {
      llmStatsModelMetadataPromise = buildLlmStatsModelMetadataIndex().catch((error) => {
