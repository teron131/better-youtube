...
function scoreValue(model: SortableModel, metric: ModelSortMetric): number | null {
  if (metric === "price") {
    return finiteNumber(model.price);
  }

  return finiteNumber(metric === "intelligence" ? model.intelligenceScore : model.speedMetric);
}
...
export function defaultModelSortDirection(metric: ModelSortMetric): ModelSortDirection {
...
export function sortModelsByRankKey<T extends SortableModel>(models: T[], key: ModelRankKey): T[] {
