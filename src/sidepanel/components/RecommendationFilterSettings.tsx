...
export function RecommendationFilterSettings() {
  ...
  const [settings, setSettings] = useState<FeedFilterSettings>(DEFAULT_FEED_FILTER_SETTINGS);
  const [history, setHistory] = useState<FilteredVideoRecord[]>([]);
  const [subscriptions, setSubscriptions] = useState<StoredSubscriptions | null>(null);
  ...
  useEffect(() => {
    ...
    const listener = (changes: Record<string, chrome.storage.StorageChange>, areaName: string) => {
      ...
      if (Object.keys(changes).some((key) => RECOMMENDATION_SETTING_STORAGE_KEYS.has(key))) {
  ...
  const handleExtractSubscriptions = async () => {
    ...
    try {
      ...
      toast({
        ...
        description: error instanceof Error ? error.message : "Failed to import subscriptions.",
        variant: "destructive",
      });
    } finally {
          ...
          >
            ...
            <span className="inline-flex items-center gap-2 whitespace-nowrap">
              <Input
                ...
                onChange={(event) =>
                  void handleSettingChange("minViews", Math.max(0, Number(event.target.value) || 0))
                }
        ...
        <div className="flex items-start justify-between gap-4">
          <div className="min-w-0 flex-1 space-y-1.5">
            ...
            <div className="text-[13px] leading-5 text-muted-foreground">
              Import your YouTube subscriptions to make the videos immune to the filters.
            </div>
          ...
          <div className="border-t border-border/60">
            {history
              ...
              .map((entry) => (
                ...
                >
                  <div className="text-sm font-medium leading-6">{entry.title}</div>
