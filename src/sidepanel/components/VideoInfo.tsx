...
function isContentPixel(data: Uint8ClampedArray, index: number): boolean {
	...
	return (
		red > BLACK_BAR_THRESHOLD ||
		green > BLACK_BAR_THRESHOLD ||
		blue > BLACK_BAR_THRESHOLD
	);
}
...
function findCropBounds(
	...
	const minContentPixelsPerRow = Math.max(
		1,
		Math.floor(width * CONTENT_ROW_RATIO),
	);
	const minContentPixelsPerCol = Math.max(
		1,
		Math.floor(height * CONTENT_COL_RATIO),
	);
