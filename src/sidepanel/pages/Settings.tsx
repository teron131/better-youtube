...
const Settings = () => {
  ...
  const [modelCostLimitInputs, setModelCostLimitInputs] = useState<ModelCostLimitInputs>(
    DEFAULT_MODEL_COST_LIMIT_INPUTS,
  );
    ...
    const loadSettings = async () => {
      try {
        ...
        setModelCostLimitInputs({
          summarizerModelCostLimit: String(nextSettings.summarizerModelCostLimit),
  ...
  const commitModelCostLimit = async (
    ...
    const nextValue = clampModelCostLimit(normalizeModelCostLimit(rawValue), priceRange);
  ...
  const setModelCostLimitInput = (key: ModelCostLimitKey, value: string) => {
    setModelCostLimitInputs((currentInputs) =>
      currentInputs[key] === value ? currentInputs : { ...currentInputs, [key]: value },
    );
  };
  ...
  const handleChange = async <K extends keyof typeof DEFAULT_SETTINGS>(
    ...
    try {
      ...
      toast({
        ...
        description: error instanceof Error ? error.message : "The setting was not saved.",
        variant: "destructive",
      });
    }
  };
    ...
    try {
      ...
      if (permissionStatus === "denied") {
        toast({
          ...
          description: "Allow this domain so the extension can call the custom LLM endpoint.",
          variant: "destructive",
        });
      ...
      toast({
        ...
        description:
          error instanceof Error ? error.message : "The browser did not grant endpoint access.",
        variant: "destructive",
      });
    }
  };
  ...
  const handleClearStoredData = async () => {
    ...
    try {
      ...
      toast({
        ...
        description: error instanceof Error ? error.message : "Cached data was not removed.",
        variant: "destructive",
      });
    } finally {
