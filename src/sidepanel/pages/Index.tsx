...
async function loadCachedVideoState(
    ...
    const [storedSummary, storedVideoInfo, storedSubtitles] = await Promise.all(
        [getSummary(videoId), getVideoMetadata(videoId), getSubtitles(videoId)],
    );
    ...
    useEffect(() => {
        const loadShowSubtitles = async () => {
            try {
                ...
                console.error(
                    "Failed to load subtitles overlay setting:",
                    error,
                );
            }
        };
    ...
    const handleCaptionSubmit = async (url: string) => {
        ...
        if (videoId) {
            try {
                ...
                console.error(
                    "Failed to load cached transcript for captions:",
                    error,
                );
            }
        }
                    ...
                    <div className="flex min-h-[var(--sidepanel-topbar-height)] items-center justify-between w-full">
                        <div className="flex items-center gap-2">
                            <Button
                                ...
                                size="sm"
                                onClick={() =>
                                    handleToggleSubtitles(!showSubtitles)
                                }
                    ...
                    <div className="space-y-4">
                        ...
                        {summaryResult?.summary && (
                            <SummaryPanel
                                ...
                                onRegenerate={
                                    isExampleMode ? undefined : handleRegenerate
                                }
