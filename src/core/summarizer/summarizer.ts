...
import { calculateScore, isAcceptable, logQuality, SUMMARY_CONFIG } from "./qualityUtils";
...
function createQualityNode() {
  return async (state: GraphState): Promise<Partial<GraphState>> => {
    ...
    return {
      quality,
      isComplete:
        score >= SUMMARY_CONFIG.MIN_QUALITY_SCORE || iterations >= SUMMARY_CONFIG.MAX_ITERATIONS,
    };
  };
}
