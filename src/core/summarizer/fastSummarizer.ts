...
import { createAgent, createMiddleware, toolStrategy } from "@/core/langgraph-web-shim";
...
function createScrapeYoutubeTool(input: SummarizationInput) {
  return tool(
    async ({ youtube_url }) => {
      ...
      const transcript = transcriptOnlyText || getTranscriptText(data.transcript ?? []);
...
export async function summarizeFast(
  ...
  const agent = createAgent({
    ...
    systemPrompt: PromptBuilder.getLlmSummaryPrompt(targetLanguage, input.title, input.description),
    responseFormat: toolStrategy(SummarySchemaNoTimestamps),
    middleware: isUrl
      ? [createGarbageFilterMiddleware(input.refinerModel ?? DEFAULTS.MODEL_REFINER)]
      : [],
  });
