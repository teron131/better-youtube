...
`src/core` is the shared, dependency-light boundary consumed by the extension's `handlers/`, `content/`, and `sidepanel/` entrypoints. Treat exported functions/constants as a compatibility surface: small signature changes can break cross-context messaging or persisted data.
...
- `src/core/constants.ts`
  ...
  - `STORAGE_KEYS`: settings keys shared across UI/handlers.
...
- `src/core/transcript/index.ts`
  - Scrape Creators client: `fetchTranscript(videoId, retries)`.
  - In-memory cache + dedupe: `transcriptCache`, `pendingTranscriptFetches`, TTL via `TIMING.TRANSCRIPT_CACHE_TTL_MS`.
- `src/core/summarizer/*`
  - Summarization workflow exports are re-exported from `src/core/summarizer/index.ts`.

## CONVENTIONS

- Prefer importing shared identifiers from `src/core/constants.ts` instead of hard-coding strings.
...
- Use `src/core/utils/chrome.ts` wrappers rather than calling `chrome.runtime.sendMessage` / `chrome.tabs.sendMessage` directly.
  - They unify error handling via `chrome.runtime.lastError`.
- Treat `src/core/storage.ts` as the only place that knows "where" data is stored.
...
- Don't change `MESSAGE_ACTIONS` or `STORAGE_KEYS` names without updating all consumers (handlers/content/sidepanel) and migration strategy.
...
- Don't bypass caching/deduplication in `transcript/index.ts`.
