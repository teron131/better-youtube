...
async function executeChromeTabExtraction(
	...
	const selectCaptionTrack = (
		...
		const isAutoGeneratedTrack = (track: ChromeTabCaptionTrack): boolean => {
			...
			const label =
				`${track.name ?? ""} ${track.trackName ?? ""}`.toLowerCase();
			if (label.includes("auto-generated")) return true;
	...
	const readVisitorData = (): string => {
		const fromYtcfg =
			(window as any).ytcfg?.get?.("VISITOR_DATA") ??
			(window as any).ytcfg?.data_?.VISITOR_DATA;
	...
	for (const fallbackClient of innertubeFallbackClients) {
		const fallbackCaptionState =
			await fetchInnertubeCaptionState(fallbackClient);
	...
	for (const captionStateForPayload of captionStatesToTry) {
		const selectedTrack = selectCaptionTrack(
			captionStateForPayload.captionTracks,
		);
		if (!selectedTrack) {
			continue;
		}
