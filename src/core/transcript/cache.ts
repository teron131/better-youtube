...
const transcriptCache = new Map<string, { data: TranscriptResponse; timestamp: number }>();
const pendingTranscriptFetches = new Map<string, Promise<TranscriptResponse | null>>();

function buildPendingTranscriptKey(videoId: string, tabId?: number): string {
  return typeof tabId === "number" ? `${videoId}::tab:${tabId}` : `${videoId}::shared`;
}

export function getCachedTranscript(videoId: string): TranscriptResponse | undefined {
...
export function setCachedTranscript(videoId: string, data: TranscriptResponse): void {
...
export function getPendingTranscript(
  ...
  return pendingTranscriptFetches.get(buildPendingTranscriptKey(videoId, tabId));
}
...
export function setPendingTranscript(
  ...
  pendingTranscriptFetches.set(buildPendingTranscriptKey(videoId, tabId), promise);
}
