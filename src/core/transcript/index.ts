...
export function toSubtitleSegments(
    ...
    return transcript.map((segment) => ({
        ...
        startTimeText:
            segment.startTimeText || formatTimestamp(segment.startMs),
    }));
}
...
export async function fetchTranscript(
    ...
    const fetchPromise = (async () => {
        ...
        const trySupadata = async () => {
            ...
            const result = await fetchTranscriptFromSupadata(
                videoId,
                supadataKey,
            );
            if (result) setCachedTranscript(videoId, result);
        ...
        const first =
            preference === "supadata" ? trySupadata : tryScrapeCreators;
        const second =
            preference === "supadata" ? tryScrapeCreators : trySupadata;
