...
function formatAgeYears(ageYears: number): string {
  const roundedYears = ageYears >= 10 ? Math.round(ageYears) : Math.round(ageYears * 10) / 10;
  return `${roundedYears} year${roundedYears === 1 ? "" : "s"}`;
}

function checkViewsFilter(videoData: VideoCardData, settings: FeedFilterSettings): FilterResult {
...
function checkMixFilter(videoData: VideoCardData, settings: FeedFilterSettings): FilterResult {
...
function checkDurationFilter(videoData: VideoCardData, settings: FeedFilterSettings): FilterResult {
  if (
...
function checkAgeFilter(videoData: VideoCardData, settings: FeedFilterSettings): FilterResult {
...
function checkKeywordsFilter(videoData: VideoCardData, settings: FeedFilterSettings): FilterResult {
  if (!settings.keywordFilterEnabled || settings.keywords.length === 0 || !videoData.title) {
    return { shouldFilter: false };
  }
...
function checkLanguageFilter(videoData: VideoCardData, settings: FeedFilterSettings): FilterResult {
...
export function hasIncompleteMetadata(
  ...
  return Boolean(
    ...
    (settings.durationFilterEnabled &&
      ...
      !videoData.duration &&
      !videoData.isActiveLiveContent) ||
    (settings.ageFilterEnabled && settings.maxAgeYears > 0 && !videoData.publishTime) ||
    (settings.keywordFilterEnabled && settings.keywords.length > 0 && !videoData.title) ||
