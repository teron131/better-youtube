...
const WATCH_LINK_SELECTOR = "a#thumbnail, a#video-title, a#video-title-link, a[href*='/watch']";
...
const METADATA_TEXT_SELECTORS = ["#metadata-line", "ytd-video-meta-block", "#channel-info"];
...
type ExtractedVideoData = Omit<VideoCardData, "titleLanguage" | "channelLanguage">;
...
export function normalizeChannelPath(path: unknown): string | null {
  ...
  try {
    ...
    if (CHANNEL_PATH_PREFIXES.some((prefix) => normalizedPath.startsWith(prefix))) {
...
export function getNormalizedChannelId(videoData: VideoCardData): string | null {
  return typeof videoData.channelId === "string" && videoData.channelId.startsWith("UC")
    ? videoData.channelId
...
export function getContainingVideoCard(node: Node): Element | null {
  if (node.nodeType === Node.ELEMENT_NODE) {
    ...
    return element.matches(VIDEO_CARD_SELECTOR) ? element : element.closest(VIDEO_CARD_SELECTOR);
  }
...
function firstNonEmpty(...values: unknown[]): string | null {
  for (const value of values) {
    const normalized = typeof value === "string" ? normalizeText(value) : textFromNode(value);
    if (normalized) {
      return normalized;
    }
  }
...
function textFromNode(value: unknown): string | null {
  ...
  return firstNonEmpty(
    ...
    node.text,
    node.label,
    Array.isArray(node.runs) ? node.runs.map((run) => textFromNode(run)).join(" ") : null,
  );
}
...
function hasLiveMetadata(
  ...
  return isStreamPublishTime(videoData.publishTime) || isWatchingCount(videoData.viewCount);
}

function hasActiveLiveMetadata(videoData: Pick<ExtractedVideoData, "viewCount">): boolean {
...
function getThumbnailOverlays(thumbnailViewModel: Record<string, unknown> | undefined): unknown[] {
  return Array.isArray(thumbnailViewModel?.overlays) ? thumbnailViewModel.overlays : [];
}

function getLockupThumbnailOverlays(lockupViewModel: Record<string, unknown>): unknown[] {
  const contentImage = lockupViewModel.contentImage as Record<string, unknown> | undefined;
...
function extractHomeDuration(lockupViewModel: unknown): string | null {
  ...
  const overlays = getLockupThumbnailOverlays(lockupViewModel as Record<string, unknown>);
...
function getVideoIdFromElement(element: Element): string | null {
  const explicitVideoId = element.querySelector("[data-video-id]")?.getAttribute("data-video-id");
  if (explicitVideoId) {
...
function extractSearchVideo(rendererData: Record<string, unknown>): ExtractedVideoData {
  ...
  const hasActiveLiveOverlay = hasLiveRendererOverlay(rendererData.thumbnailOverlays);

  return {
    videoId: typeof rendererData.videoId === "string" ? rendererData.videoId : null,
    title: textFromNode(rendererData.title),
    duration: normalizeDurationText(rendererData.lengthText),
    viewCount,
    publishTime,
    isLiveContent: hasLiveMetadata({ publishTime, viewCount }) || hasActiveLiveOverlay,
    isActiveLiveContent: hasActiveLiveMetadata({ viewCount }) || hasActiveLiveOverlay,
...
function extractLockupVideo(lockupViewModel: Record<string, unknown>): ExtractedVideoData {
  ...
  const commandRuns = Array.isArray(uploaderPart?.commandRuns) ? uploaderPart.commandRuns : [];
  const homeChannelInfo =
    commandRuns.length > 0
      ? getChannelInfoFromEndpoint(
          (commandRuns[0] as { onTap?: { innertubeCommand?: unknown } }).onTap?.innertubeCommand,
        )
      : { channelId: null, channelPath: null };
  const publishTime = findMetadataText(metadataTexts, PUBLISH_TIME_PATTERN);
  const hasActiveLiveOverlay = hasLiveRendererOverlay(getLockupThumbnailOverlays(lockupViewModel));

  return {
    videoId: typeof lockupViewModel.contentId === "string" ? lockupViewModel.contentId : null,
    ...
    publishTime,
    isLiveContent: hasLiveMetadata({ publishTime, viewCount: null }) || hasActiveLiveOverlay,
...
function extractVideoFromRenderer(rendererData: unknown): ExtractedVideoData | null {
...
function fillMetadataFromFullText(videoData: ExtractedVideoData, fullText: string | null) {
  if (!fullText) {
    return;
  }
...
function detectActiveLiveContentFromElement(videoElement: Element): boolean {
  for (const element of videoElement.querySelectorAll(LIVE_INDICATOR_SELECTOR)) {
    const indicatorText = normalizeText(element.getAttribute("aria-label") || element.textContent);
...
function isYouTubeGeneratedMixCard(videoElement: Element, title: string | null): boolean {
  if (!title || !/^mix\s*-/i.test(title)) {
    return false;
  }

  const linkHref = videoElement.querySelector(WATCH_LINK_SELECTOR)?.getAttribute("href");
...
function fillChannelInfoFromLink(videoElement: Element, videoData: ExtractedVideoData) {
  ...
  if (!videoData.channelName) {
    videoData.channelName =
      normalizeText(channelLink.textContent) || normalizeText(channelLink.getAttribute("title"));
  }
}

function fillLockupChannelNameFromText(videoElement: Element, videoData: ExtractedVideoData) {
...
function getStructuredVideoData(videoElement: Element): ExtractedVideoData | null {
...
export function extractVideoData(videoElement: Element): VideoCardData {
  ...
  try {
    ...
    if (!data.isActiveLiveContent) {
      data.isActiveLiveContent = detectActiveLiveContentFromElement(videoElement);
    }
    if (data.isActiveLiveContent) {
