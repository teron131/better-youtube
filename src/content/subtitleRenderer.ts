...
const SUBTITLE_SYNC_LOOKAHEAD_MS = 120;
...
class SubtitleController {
  ...
  private syncPlayback = (): void => {
    ...
    const nextSyncDelay = this.getNextSyncDelay(currentTime, nextSubtitleIndex);
  ...
  private getNormalizedSubtitleText(subtitleIndex: number): string {
    ...
    const normalizedText = normalizeSubtitleText(this.subtitles[subtitleIndex]?.text || "");
  ...
  private getNextSyncDelay(currentTimeMs: number, activeSubtitleIndex: number): number {
    const nextSubtitleStartMs =
      this.subtitles[this.findLatestStartedSubtitleIndex(currentTimeMs) + 1]?.startTime ?? null;
    const activeSubtitle = this.subtitles[activeSubtitleIndex];
    const activeSubtitleEndMs =
      activeSubtitle && currentTimeMs < activeSubtitle.endTime ? activeSubtitle.endTime : null;
    const nextBoundaryMs =
      nextSubtitleStartMs === null
        ? activeSubtitleEndMs
        : activeSubtitleEndMs === null
          ? nextSubtitleStartMs
          : Math.min(activeSubtitleEndMs, nextSubtitleStartMs);
    ...
    const mediaDelayMs = nextBoundaryMs - currentTimeMs;
    const wallClockDelayMs = (mediaDelayMs - SUBTITLE_SYNC_LOOKAHEAD_MS) / playbackRate;
