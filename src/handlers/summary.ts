...
import { isGeminiModelSelection, resolveSummarizationRoute } from "@/core/workRouter";
...
function normalizeModePreference(input: {
  ...
  if (summarizerMode === "native" || summarizerMode === "validation" || summarizerMode === "fast") {
...
async function broadcastSummaryResult(
  ...
  await saveSummary(videoId, result.summary, modelSelection, targetLanguage, result.quality);
...
function buildSummaryWorkloadKey(input: {
  ...
  const qualityModelKey = String(input.qualityModel || input.modelSelection || "");
...
async function runSummaryJob(input: {
  ...
  setTranscriptFetchContext(videoId, transcriptContextOwnerId, transcriptFetchContext);
  try {
    ...
    const providerRunners: Record<ConcreteProvider, () => Promise<SummaryResult>> = {
      gemini: tryGemini,
      llm: tryLlm,
    };
