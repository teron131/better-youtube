...
import type { StoredSubscriptions, SubscriptionRecord } from "@/core/recommendationFilters";
...
export async function handleExtractSubscriptions(
  ...
  try {
    ...
    sendResponse({
      success: false,
      error: error instanceof Error ? error.message : "Failed to extract subscriptions.",
    });
  }
}
...
async function runPageSubscriptionExtraction(tabId: number): Promise<SubscriptionRecord[]> {
...
function extractSubscriptionsInPageContext() {
  ...
  function normalizeChannelPath(path: unknown) {
    ...
    try {
      ...
      if (normalizedPath.startsWith("/@") || normalizedPath.startsWith("/channel/")) {
  ...
  function chooseBetterName(existingName: string | null, nextName: string | null) {
    ...
    return normalizedNext.length < normalizedExisting.length ? normalizedNext : normalizedExisting;
  }
  ...
  function buildSubscriptionRecord(input: {
    ...
    const normalizedName = isPlausibleChannelName(input.name) ? normalizeText(input.name) : null;
    ...
    return {
      ...
      channelUrl: normalizedPath ? new URL(normalizedPath, window.location.origin).href : null,
  ...
  function mergeChannelRecords(
    ...
    return {
      ...
      channelPath: nextChannel.channelPath || existingChannel.channelPath || null,
      channelUrl: nextChannel.channelUrl || existingChannel.channelUrl || null,
      handle: nextChannel.handle || existingChannel.handle || null,
      description: nextChannel.description || existingChannel.description || null,
    };
  }
  ...
  function getChannelRecordFromRenderer(renderer: Element): PageSubscriptionRecord | null {
    ...
    return buildSubscriptionRecord({
      name:
        ...
        normalizeText(renderer.querySelector(`#main-link, ${CHANNEL_LINK_SELECTOR}`)?.textContent),
  ...
  async function scrollToLoadAllChannels() {
    ...
    for (let pass = 0; pass < MAX_SCROLL_PASSES; pass += 1) {
      ...
      const currentChannelCount = document.querySelectorAll(CHANNEL_RENDERER_SELECTOR).length;
  ...
  async function extractAllSubscriptions() {
    ...
    const renderers = Array.from(document.querySelectorAll(CHANNEL_RENDERER_SELECTOR));

    for (const renderer of renderers) {
      const nextChannel =
        getChannelRecordFromRenderer(renderer) || getChannelRecordFromDom(renderer);
      ...
      const key =
        nextChannel.channelId || nextChannel.channelPath || nextChannel.handle || nextChannel.name;
      if (!key) {
        continue;
      }

      const existing = channels.get(key);
      channels.set(key, existing ? mergeChannelRecords(existing, nextChannel) : nextChannel);
    }

    return Array.from(channels.values()).sort((left, right) =>
      (left.name || left.channelPath || "").localeCompare(right.name || right.channelPath || ""),
    );
  }
